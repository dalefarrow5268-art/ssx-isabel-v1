...
export default function UnrealLiveOfficePage() {
    ...
    <main style={{ minHeight: '100vh', background: '#0a0c10', color: '#f4f5f7', padding: 24, fontFamily: 'Arial, sans-serif' }}>
      <div style={{ maxWidth: 1500, margin: '0 auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'center', marginBottom: 18 }}>
          <div>
            <div style={{ fontSize: 13, opacity: 0.7, letterSpacing: 1.4 }}>SSX • ISABEL • OPTIONAL RENDERER</div>
            <h1 style={{ margin: '5px 0 0', fontSize: 28 }}>Unreal / Pixel Streaming Renderer</h1>
          </div>
        ...
        <section style={{ position: 'relative', aspectRatio: '16 / 9', border: '1px solid #30343b', background: '#11151b', overflow: 'hidden' }}>
          {streamUrl ? (
            ...
            <div style={{ height: '100%', display: 'grid', placeItems: 'center', textAlign: 'center', padding: 30 }}>
              <div>
                <div style={{ fontSize: 24, marginBottom: 10 }}>Optional Unreal renderer is offline</div>
                <div style={{ maxWidth: 700, opacity: 0.72, lineHeight: 1.55 }}>
                  Isabel now has a Cloudflare-first browser runtime. This page is preserved for a future high-end Unreal / MetaHuman renderer when dedicated GPU hardware is available.
                </div>
              </div>
            </div>
          )}
        ...
        <section style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 16 }}>
          {([
            ...
            <button key={command} onClick={() => sendCommand(command)} style={{ padding: '10px 14px', border: '1px solid #3d434c', background: '#171b22', color: '#fff', cursor: 'pointer' }}>
              {label}
            </button>
          ))}
        </section>
