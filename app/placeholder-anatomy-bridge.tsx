  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const enhanceRig = (rig: import("three").Object3D) => {
        if (disposed || rig.children.some((child) => child.name === "HUMAN_PLACEHOLDER_V2")) return;

        const object = rig.children.find((child) =>
        ...
        // Hide the original oversized capsule torso.
        for (const child of rig.children) {
          if ((child as import("three").Mesh).isMesh && child !== object && Math.abs(child.position.y - 2.52) < 0.2) child.visible = false;
        }

        // Hide the original tube-limb groups.
        for (const child of rig.children) {
        ...
        // Human head proportions and visible neck separation.
        ...
        // Face direction cues. The existing eyes and mouth stay untouched.
      ...
      objectPrototype.add = function patchedAdd(...objects: import("three").Object3D[]) {
        ...
        if (this.name === "PROCEDURAL_PLACEHOLDER_NOT_ISABEL" && !pending.has(this)) {
          pending.add(this);
          // createPlaceholder builds the head contents after adding the empty pivot.
          // Defer inspection until the synchronous construction pass has finished.
          queueMicrotask(() => {
            pending.delete(this);
            enhanceRig(this);
          });
