...
const includesNamed = (values: string[], required: string) => values.some((value) => normalizeName(value).includes(normalizeName(required)));
...
function inspectAsset(root: Object3D, animations: AnimationClip[]) {
  ...
  root.traverse((object) => {
    ...
    if ((object as Mesh).isMesh) {
      ...
      materials.forEach((material) => { if (material && "map" in material && material.map) texturedMaterials += 1; });
    }
  });
...
function createPlaceholder(THREE: typeof import("three")) {
  ...
  makeLimb(-0.72, 2.92, 0.15, 0.72); makeLimb(0.72, 2.92, 0.15, 0.72);
  makeLimb(-0.28, 1.72, 0.19, 0.88); makeLimb(0.28, 1.72, 0.19, 0.88);
  return { rig, torso, headPivot, eyes, mouth };
}

function drawMonitor(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement, detail: ScreenAction | null, waiting: boolean) {
  ...
  context.strokeStyle = accent; context.lineWidth = 8; context.strokeRect(18, 18, canvas.width - 36, canvas.height - 36);
  context.fillStyle = accent; context.font = "700 30px Arial"; context.fillText(waiting ? "AUTHORIZATION REQUIRED" : "ISABEL ACTIVE DISPLAY", 48, 72);
  context.fillStyle = "#ffffff"; context.font = "700 48px Arial";
  context.fillText(detail ? `${detail.screen.toUpperCase()} · ${detail.action.toUpperCase()}` : "MONITOR STANDING BY", 48, 150);
  context.fillStyle = "#b9d4df"; context.font = "32px Arial"; context.fillText(detail?.recordId ?? "No active record", 48, 215);
...
export default function ThreeMotionLab() {
  ...
  useEffect(() => { window.dispatchEvent(new CustomEvent("isabel-three-state", { detail: step.state })); }, [step.state]);
  useEffect(() => { window.dispatchEvent(new CustomEvent("isabel-camera-mode", { detail: cameraMode })); }, [cameraMode]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      renderer.shadowMap.enabled = true; renderer.outputColorSpace = THREE.SRGBColorSpace; mount.appendChild(renderer.domElement);
      scene.add(new THREE.HemisphereLight(0xcde7ff, 0x1d1712, 1.65));
      const key = new THREE.DirectionalLight(0xfff3df, 3.4); key.position.set(4, 8, 5); key.castShadow = true; scene.add(key);
      const amber = new THREE.PointLight(0xffa84d, 26, 18); amber.position.set(5.8, 3.8, 1.8); scene.add(amber);
      ...
      const backWall = new THREE.Mesh(new THREE.BoxGeometry(15, 6.8, 0.32), new THREE.MeshStandardMaterial({ color: 0x11171e })); backWall.position.set(0, 3.35, -4.45); scene.add(backWall);
      const sideWall = new THREE.Mesh(new THREE.BoxGeometry(0.28, 6.8, 10), new THREE.MeshStandardMaterial({ color: 0x121820 })); sideWall.position.set(7.35, 3.35, 0.15); scene.add(sideWall);
      const accent = new THREE.Mesh(new THREE.BoxGeometry(0.14, 4.8, 6.4), new THREE.MeshStandardMaterial({ color: 0xb56a25, emissive: 0x4a1f08 })); accent.position.set(7.15, 3.2, -0.45); scene.add(accent);

      const monitorCanvas = document.createElement("canvas"); monitorCanvas.width = 1024; monitorCanvas.height = 576;
      const monitorContext = monitorCanvas.getContext("2d"); if (!monitorContext) throw new Error("Unable to create monitor canvas context");
      drawMonitor(monitorContext, monitorCanvas, null, false);
      const monitorTexture = new THREE.CanvasTexture(monitorCanvas); monitorTexture.colorSpace = THREE.SRGBColorSpace; monitorTexture.needsUpdate = true;
      const monitors: Mesh[] = [];
      [-1, 0, 1].forEach((position, monitorIndex) => {
        const frameMesh = new THREE.Mesh(new THREE.BoxGeometry(3.05, 1.95, 0.18), new THREE.MeshStandardMaterial({ color: 0x050709, metalness: 0.75 })); frameMesh.position.set(position * 3.45, 3.65, -4.16); scene.add(frameMesh);
        const material = monitorIndex === 0 ? new THREE.MeshStandardMaterial({ map: monitorTexture, emissive: 0x174e60, emissiveIntensity: 1.4, roughness: 0.22 }) : new THREE.MeshStandardMaterial({ color: 0x101b23, emissive: [0x15677f, 0x675025, 0x294f61][monitorIndex], emissiveIntensity: 1.25 });
        const screen = new THREE.Mesh(new THREE.BoxGeometry(2.82, 1.7, 0.08), material); screen.position.set(position * 3.45, 3.65, -4.04); scene.add(screen); monitors.push(screen);
      });
      const wood = new THREE.MeshStandardMaterial({ color: 0x5a3b27, roughness: 0.46 });
      const desk = new THREE.Mesh(new THREE.BoxGeometry(5.2, 0.24, 1.7), wood); desk.position.set(0.4, 1.2, 2.15); scene.add(desk);
      [-1.75, 2.55].forEach((x) => { const leg = new THREE.Mesh(new THREE.BoxGeometry(0.22, 1.2, 1.35), wood); leg.position.set(x, 0.58, 2.15); scene.add(leg); });

      const placeholder = createPlaceholder(THREE);
      let character: Group = placeholder.rig;
      const jawBindings: MorphBinding[] = [];
      character.position.set(0.4, -0.72, 1.1); scene.add(character); setAssetReport({ phase: "loading", source: MODEL_PATH, checks: [] });
      try {
        ...
        scene.remove(character); character = gltf.scene; character.name = "ISABEL_PRODUCTION_GLB";
        const size = new THREE.Box3().setFromObject(character).getSize(new THREE.Vector3()); character.scale.setScalar(size.y > 0 ? 4.15 / size.y : 1);
        const calibrated = new THREE.Box3().setFromObject(character); character.position.set(0.4, -calibrated.min.y - 0.72, 1.1);
        character.traverse((object) => {
          if ((object as Mesh).isMesh) {
            const mesh = object as Mesh; mesh.castShadow = true; mesh.receiveShadow = true;
            const dictionary = mesh.morphTargetDictionary ?? {};
            const jawName = Object.keys(dictionary).find((name) => normalizeName(name).includes("jawopen"));
            if (jawName !== undefined) jawBindings.push({ mesh, index: dictionary[jawName] });
          }
        });
        scene.add(character); setAssetReport({ phase: "ready", source: MODEL_PATH, checks });
      } catch (error) {
        setAssetReport({ phase: "placeholder", source: MODEL_PATH, error: error instanceof Error ? error.message : "Unknown model-loading error", checks: [{ label: "Production GLB", ok: false, detail: "placeholder active" }] });
      }
      ...
      const confirmationListener = () => { confirmationWaiting = true; drawMonitor(monitorContext, monitorCanvas, activeScreen, true); monitorTexture.needsUpdate = true; };
      ...
      const animate = () => {
        ...
        if (activeState === "working" || activeState === "notice" || activeState === "sit") {
          ...
          const returning = activeState === "return"; const targetX = returning ? 0.4 : -2.75; const targetZ = returning ? 1.1 : -1.45;
          const dx = targetX - character.position.x; const dz = targetZ - character.position.z;
          character.position.x = approach(character.position.x, targetX, 0.014); character.position.z = approach(character.position.z, targetZ, 0.014); character.position.y = Math.abs(Math.sin(t * 7.2)) * 0.04; character.rotation.y = approach(character.rotation.y, Math.atan2(dx, dz), 0.07);
        } else if (activeState === "present") {
          character.position.x = approach(character.position.x, -2.75, 0.05); character.position.z = approach(character.position.z, -1.45, 0.05); character.position.y = approach(character.position.y, 0, 0.08); character.rotation.y = approach(character.rotation.y, Math.PI, 0.06);
        } else if (activeState === "listen") character.rotation.y = approach(character.rotation.y, 0.35, 0.055);
  ...
  const chooseState = (next: MotionState) => { setRunning(false); setIndex(sequence.findIndex((item) => item.state === next)); };
  const passed = assetReport.checks.filter((check) => check.ok).length;
  return (
    <main className={styles.page}><section className={styles.stage}>
      ...
      <aside className={styles.panel}><span>ACTIVE PRESENCE STATE</span><h1>{step.title}</h1><p>{step.detail}</p>
        ...
        <div className={styles.timeline}>{sequence.map((item, itemIndex) => <button key={item.state} className={itemIndex === index ? styles.current : ""} onClick={() => chooseState(item.state)}><b>{String(itemIndex + 1).padStart(2, "0")}</b><span>{item.title}</span></button>)}</div>
        <dl><div><dt>Asset phase</dt><dd>{assetReport.phase}</dd></div><div><dt>Model path</dt><dd>{assetReport.source}</dd></div><div><dt>Validation</dt><dd>{assetReport.checks.length ? `${passed}/${assetReport.checks.length}` : "pending"}</dd></div><div><dt>Camera</dt><dd>{cameraMode}</dd></div></dl>
