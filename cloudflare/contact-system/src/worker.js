...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/health') return json({ ok:true, service:'ssx-contact-system', version:'v4' });
...
function withCountdowns(tasks){const now=Date.now();return tasks.map(task=>({...task,...countdown(task.due_at,now)}));}
function countdown(dueAt,now){if(!dueAt)return{remaining_seconds:null,remaining_time:null,is_overdue:false};const dueMs=Date.parse(dueAt);if(Number.isNaN(dueMs))return{remaining_seconds:null,remaining_time:null,is_overdue:false};const remainingSeconds=Math.trunc((dueMs-now)/1000);return{remaining_seconds:remainingSeconds,remaining_time:humanDuration(Math.abs(remainingSeconds)),is_overdue:remainingSeconds<0};}
function humanDuration(totalSeconds){const minutes=Math.max(0,Math.ceil(totalSeconds/60));if(minutes<60)return `${minutes} min`;const hours=Math.floor(minutes/60);const mins=minutes%60;if(hours<24)return mins?`${hours} hours ${mins} min`:`${hours} hours`;const days=Math.floor(hours/24);const remHours=hours%24;return remHours?`${days} days ${remHours} hours`:`${days} days`;}
